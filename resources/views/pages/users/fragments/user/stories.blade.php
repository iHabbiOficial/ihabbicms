...
<x-title-box
    icon="story-camera"
    title="{{ __('Friends Stories') }} ({{ $randomStories }}) - Em breve!"
    description="{{ __('Stay on top of your friends latest activities') }}"
/>
<div class="flex gap-3 scroll-smooth scroll-x relative mt-4 p-2 pb-6 overflow-x-auto rounded-lg shadow border-b-2 border-gray-300 dark:border-slate-800 bg-white dark:bg-slate-950" style="opacity: .5;">
    @for ($i = 0; $i < $randomStories; $i++)
        <div
            data-tippy-singleton
            data-tippy-content="<small>2 minutos atrás</small>"
            @class([
            ...
            <div class="absolute max-w-[100%] truncate text-xs -bottom-5 left-1/2 -translate-x-1/2 dark:text-slate-200">iHabbi</div>
        </div>
    @endfor
</div>
