            ...
            @foreach ($photos as $photo)
                <div class="bg-white dark:bg-slate-950 p-2 rounded-lg border-b-2 border-gray-300 dark:border-slate-800 shadow-lg h-auto">
                    <div
                        ...
                        data-src="{{ $photo->url }}"
                        data-sub-html='<h4>{{ __("Photo by") }} <a href="#" class="underline underline-offset-4">{{ $photo->user->username }}</a></h4><p>{{ __("Photo taken on") }} <b>{{ $photo->formattedDate }}</b> {{ __("in the room") }} <a href="#" class="underline underline-offset-4">{{ optional($photo->room)->name }}</a></p>'
                    ...
                    <div class="w-full flex justify-start items-center gap-3 p-1 bg-gray-100 rounded-b-lg border-t-2 border-gray-300 dark:border-slate-600 dark:bg-gray-900">
                        <div
                            ...
                            style="background-image: url('{{ getSetting('figure_imager') . $photo->user->look }}&direction=2&head_direction=2&size=m&gesture=sml&headonly=1')"
                        ></div>
